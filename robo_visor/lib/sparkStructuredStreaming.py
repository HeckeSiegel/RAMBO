...
class backtest:
    ...
    def full_backtest(self, startCap, startCap_market, commission, risk_free, strategy, interval, start, end, period, n_stock, hdfs_path, sqlContext, best_of, mode):
        ...
        if(mode == "full"):
            # do full backtest for each day in time period and simulate trading for each day
            ...
            self.performance_loop(df_daily_loop, df_minute, period, startCap, startCap_market, commission, risk_free, strategy, interval, start, end, n_stock, hdfs_path, sqlContext,hdfs_dir, best_of)
            self.performance_best(df_daily_best, startCap, startCap_market, start, end, commission, risk_free, interval, hdfs_path, sqlContext, hdfs_dir)
            
        if(mode == "no_best"):
            # without trading simulation
            df_daily_loop = self.get_historical_daily(None, end, n_days)
            df_minute = self.get_historical_minute(None, start, end)
            self.performance_loop(df_daily_loop, df_minute, period, startCap, startCap_market, commission, risk_free, strategy, interval, start, end, n_stock, hdfs_path, sqlContext,hdfs_dir, best_of)
            
        if(mode == "only_best"):
            # only trading simulation without the full backtest for every day
            # can only be used if there is data in /backtest/performance_full/yyyymmdd
            df_daily_best = self.get_historical_daily(None, end+timedelta(days=1), n_days+1)
            self.performance_best(df_daily_best, startCap, startCap_market, start, end, commission, risk_free, interval, hdfs_path, sqlContext, hdfs_dir)
...
class realtime:
    ...
    def realtime(self, startCap, portfolio_size, commission, sandbox,sqlContext):
        ...
        print("1. Find out which strategy worked best yesterday")
        print("")
        print("2. Use that strategy to find stocks for today")
        print("")
        #best_params = self.get_best_performance(day, sqlContext)
        symbol = ["ABBV","ZBH"]
        share = [0.5,0.5]
        strategy_name = "momentum60"
        
        # write and start producer for given symbols
        print("3. Start Kafka Producer")
        print("")
        self.write_producer_batch(symbol, sandbox)
        subprocess.call(['producer_start.bat'])
        
        time.sleep(5*10*len(symbol)*2)
        ...
        value, date_list, moneyForInvesting_list, moneyInStocks_list, stocksOwned, trades_total, cash = init
        ...
        while(True):
            last_datetime = date_list[0]
            
            depot = self.realtime_loop(value, date_list, moneyForInvesting_list, moneyInStocks_list, stocksOwned,\
                                               trades_total, symbol, share, commission, strategy, sqlContext, directory)
                    
            value, date_list, moneyForInvesting_list, moneyInStocks_list, stocksOwned, trades_total, cash = depot
                
            current_datetime = date_list[0]
